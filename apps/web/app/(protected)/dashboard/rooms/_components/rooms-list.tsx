...
function formatRelativeTime(value: string) {
  ...
  const amount =
    diffMs < 0 ? Math.floor(absMs / unit.ms) : Math.ceil(absMs / unit.ms);
  return diffMs < 0
...
export function RoomsList({ selectedRoomId, onSelect }: RoomsListProps) {
  const {
    data,
    fetchNextPage,
    hasNextPage,
  ...
  const seletedRoomName = rooms.find(room => room.id === selectedRoomId)?.name;
            ...
            rooms.map(room => {
              ...
              return (
                <div
                  key={room.id}
                  role="button"
                  tabIndex={expires.expired ? -1 : 0}
                  onClick={() => !expires.expired && onSelect?.(room.id)}
                  className={cn(
                    'px-4 transition-colors',
                    expires.expired
                      ? 'opacity-50 cursor-not-allowed'
                      : isSelected
                        ? 'bg-blue-50 border border-blue-400 cursor-pointer'
